    ...
    public partial class AccountInfo : UserControl
    {
        ...
        private void DrawPaint(object sender, PaintEventArgs e)
        {
            if(sender is Label label)
            {
                ...
                if (BackgroundImage != null) Core.General.UI.DrawControl.DrawBackground(e.Graphics, BackgroundImage, this, label);
                Core.General.UI.DrawControl.DrawOutlineString(e.Graphics, label, label.ForeColor, Color.Black, 2);
            }
        }
        ...
        private async void ControlLoad(object sender, EventArgs e)
        {
            ...
            RawImage = url == null ? null : await Core.WebRequest.ImageGetRequest(url);
            DrawBackground();
        }
