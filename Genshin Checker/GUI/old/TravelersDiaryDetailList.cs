...
namespace Genshin_Checker.Window
{
    public partial class TravelersDiaryDetailList : Form
    {
        ...
        private void UpdateDataMonth()
        {
            ...
            foreach (var a in PathList) monthlist.Items.Add(string.Format(Common.FormatYearMonth, a.year, Core.General.LocalizeValue.Convert.MonthShort(a.month)));
            if (monthlist.Items.Count > 0) monthlist.SelectedIndex = 0;
        }
            ...
            try
            {
                ...
                try
                {
                    if (gameeventpath != null && AppData.IsExistFile(gameeventpath))
                        gameeventlists = JsonChecker<Model.UserData.GameDatabase.NameLocalize.Root>.Check(await Core.General.AppData.LoadUserData(gameeventpath) ?? "{}");
                }
                catch { }
                //HoYoLabの方のローカライズを取得
                try
                {
                    if (eventpath != null && AppData.IsExistFile(eventpath))
                        eventlists = JsonChecker<EventName>.Check(await Core.General.AppData.LoadUserData(eventpath) ?? "");
                }
                catch { }
                //カスタマーセンターのデータベースを取得
                try
                ...
                {
                    var typename = Core.General.TravelersDiaryDatailEventConverter.GetEventName(a.EventType, eventlists);
                ...
                foreach (var a in gamelists.Details)
                {
                    ...
                    if (a.Count < 0 || id != int.MinValue || lists.Details.Find(b => b.EventTime == a.EventTime) == null)
                    {
                        ...
                        var typename = Core.General.TravelersDiaryDatailEventConverter.GetEventName(a.EventTypeID, a.EventType, gameeventlists);
                ...
                if (IsEventIDChanged && expath != null)
                    await Core.General.AppData.SaveUserData(expath, JsonConvert.SerializeObject(gamelists));
        ...
        private void TravelersDiaryDetail_ProgressChanged(object? sender, TravelersDiaryDetail.ProgressState e)
        {
            ...
            toolStripStatusLabel1.Text = $"{p:0.0}% {string.Format(Localize.WindowName_TravelersDiaryDetailList_Downloading, e.month == 0 ? Localize.WindowName_TravelersDiaryDetailList_Downloading_ThisMonth : Core.General.LocalizeValue.Convert.MonthShort(e.month), e.mode, e.CurrentPage)}";
        ...
        private void dataGridView1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                ...
                if (type != null)
                {
                    switch (Core.General.TravelersDiaryDatailEventConverter.GetEventType((int)type.Value))
                    {
        ...
        {
            Core.General.ManageWindow.OpenWindow(null, nameof(Window.ProgressWindow.LoadGameDatabase));
        }
    }
}
