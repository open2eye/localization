...
namespace Genshin_Checker.Window
{
    public partial class RealTimeData : Form
    {
            ...
            if (Note.Meta.Message == "OK")
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    if (r.Expedition.Expeditions.Count > i)
                    {
                        ...
                        if (data.ImageURL != ex_icon[i].Name) ex_icon[i].Image = await Core.WebRequest.ImageGetRequest(data.ImageURL);
        ...
        private async void WindowLoad(object sender, EventArgs e)
        {
            ...
            var image = url == null ? null : await Core.WebRequest.ImageGetRequest(url);
