...
namespace Genshin_Checker.Core.HoYoLab
{
    public class TravelersDiaryDetail : Base
    {
        ...
        private async Task CorrectData(int month, Mode mode, int position = 0, int total = int.MaxValue)
        {
            ...
            try
            {
                var namelist = await AppData.LoadUserData(eventpath);
                if (JsonChecker<EventName>.IsValid(namelist) && namelist != null)
                    eventNames = JsonChecker<EventName>.Check(namelist);
                else eventNames = new();
            }
            ...
            for (int i = 1; i <= MAXPAGE; i++)
            {
                ...
                if (i == 1) //最初の場合のみの処理、ここで詳細な日付の解析をする。
                {
                    if (data.List.Count != 0)
                    {
                        ...
                        try
                        {
                            eventLists = JsonChecker<EventLists>.Check(await Core.General.AppData.LoadUserData(path)??"{}");
                        }
