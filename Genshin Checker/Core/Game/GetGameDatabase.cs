    ...
    public class GetGameDatabase
    {
        ...
        private async Task GetEquipmentList(DataType type, int year, int month)
        {
                ...
                if (i == 1)
                {
                    if (data.list.Count != 0)
                    {
                        ...
                        if (AppData.IsExistFile(path))
                            eventLists = JsonChecker<Model.UserData.GameDatabase.Enquipment.Root>.Check(await Core.General.AppData.LoadUserData(path) ?? "{}");
            ...
            if (LocaleQueue.Count > 0)
            {
                ...
                if (path != null) await Core.General.AppData.SaveUserData(localizePath, JsonConvert.SerializeObject(localize));
            ...
            if (LocaleQueueEnquipment.Count > 0)
            {
                ...
                if (path != null) await Core.General.AppData.SaveUserData(localizePathEnquipment, JsonConvert.SerializeObject(Enquipmentlocalize));
        ...
        private async Task GetMonthCardList(DataType type, int year, int month)
        {
            ...
            Dictionary<string, long> LocaleQueue = new();//注:longの値は実行する用のend_idです。
            if (AppData.IsExistFile(localizePath))
                localize = JsonChecker<Model.UserData.GameDatabase.NameLocalize.Root>.Check(await Core.General.AppData.LoadUserData(localizePath) ?? "{}");
            ...
            for (int i = 1; i <= MAXPAGE; i++)
            {
                ...
                if (i == 1)
                {
                    if (data.list.Count != 0)
                    {
                        ...
                        Log.Debug($"データベースのパスを取得しました。{year}/{month}({type}) : {path}");
                        if (AppData.IsExistFile(path))
                            eventLists = JsonChecker<Model.UserData.GameDatabase.MonthlyCard.Root>.Check(await Core.General.AppData.LoadUserData(path) ?? "{}");
                        else Log.Debug($"データベースは見つかりませんでした。");
                        if (eventLists != null && eventLists.Details.Count > 0) //イベントリストの下準備
                        {
                            eventLists.Details.Sort((a, b) => a.EventTime.CompareTo(b.EventTime));      //リストを獲得時刻昇順に並び替え
                ...
                if (IsEnd || i == MAXPAGE)
                {
                    ...
                    for (int r = 0; r < LatestCount && eventLists.Details.Count > 0; r++) eventLists.Details.RemoveAt(eventLists.Details.Count - 1);
                    eventLists.Details.Sort((a, b) => a.EventTime.CompareTo(b.EventTime));
                    if (path != null) await Core.General.AppData.SaveUserData(path, JsonConvert.SerializeObject(eventLists));
            ...
            for (int i = 1; i <= MAXPAGE; i++)
            {
                ...
                if (i == 1)
                {
                    if (data.list.Count != 0)
                    {
                        ...
                        Log.Debug($"データベースのパスを取得しました。{year}/{month}({type}) : {path}");
                        if (AppData.IsExistFile(path))
                            eventLists = JsonChecker<Model.UserData.GameDatabase.ItemNum.Root>.Check(await Core.General.AppData.LoadUserData(path) ?? "{}");
                        else Log.Debug($"データベースは見つかりませんでした。");
                        if (eventLists != null && eventLists.Details.Count > 0) //イベントリストの下準備
                        {
                            eventLists.Details.Sort((a, b) => a.EventTime.CompareTo(b.EventTime));      //リストを獲得時刻昇順に並び替え
